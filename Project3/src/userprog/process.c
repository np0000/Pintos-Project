...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  // init supplementary page table for current thread
  page_table_init(t);
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      struct sup_page_table_entry* sup = page_alloc_for_segment(upage, writable, file, ofs, page_read_bytes, page_zero_bytes);
...
setup_stack (void **esp, char *file_name) 
{
  ...
  struct sup_page_table_entry *page = page_alloc_for_stack(((uint8_t *) PHYS_BASE) - PGSIZE, true);
  if (page == NULL){
    return false;
  }

  // get a page and a frame
  page->frame_table_entry = frame_alloc(page);
  ...
  *esp -= 4;
  ...
  frame_unlock(page->frame_table_entry);
