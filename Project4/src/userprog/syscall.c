...
syscall_handler (struct intr_frame *f) 
{
  ...
  switch (syscall_number){
    ...
    case SYS_READDIR:
    {
      check_pointer((void*)(((int*)f->esp + 2)), sizeof(char*));
      int fd = *((int*)f->esp + 1);
      char *dir = (char*)(*((int*)f->esp + 2));
      int ret = readdir(fd, dir);
      f->eax = (uint32_t)ret;
      break;
    }
    case SYS_ISDIR:
...
bool create(const char *file, unsigned initial_size){
  ...
  if (file != NULL)
    {
      flag = filesys_create(file, initial_size, false);
    }
  return flag;
}
...
void close (int fd){
  ...
  {  
    ...
    struct inode *inode = file_get_inode(ffd->file);
...
bool mkdir(const char *dir){
  ...
  if (dir != NULL)
    {
      flag = filesys_create(dir, 0, true);
    }
  return flag;
}
...
bool readdir(int fd, char *name){
  ...
  struct dir *dir = (struct dir *)ffd->file;
  flag = dir_readdir(dir, name);
  dir_is_empty(dir);
